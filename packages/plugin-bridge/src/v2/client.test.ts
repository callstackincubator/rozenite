...
import { createMockChannel, createMockChannelPair } from './test-utils/mock-channel.js';
...
describe('Plugin Bridge v2', () => {
  let deviceChannel: ReturnType<typeof createMockChannel>;
  let panelChannel: ReturnType<typeof createMockChannel>;
  ...
  describe('Manual-ready mode', () => {
    it('should not start handshake until makeReady is called', async () => {
      ...
      // Call makeReady
      (deviceClient as any).makeReady();
      (panelClient as any).makeReady();
  ...
  describe('Edge cases', () => {
    ...
    it('should handle sending multiple message types before handshake', async () => {
      ...
      deviceClient.onMessage('test-event', (payload) => testEvents.push(payload));
      deviceClient.onMessage('another-event', (payload) => anotherEvents.push(payload));
      panelClient.onMessage('test-event', () => {});
