...
const isIgnorableBridgeIngestError = (event: string, error: unknown): boolean => {
  ...
  return error.message.includes('Unexpected profiling data update from renderer');
};
    ...
    return ensureBridge(state).then((bridge) => {
      while (state.pendingMessages.length > 0) {
        ...
        try {
          ...
          if (isIgnorableBridgeIngestError(next.event, error)) {
            continue;
          }
          throw error;
        }
