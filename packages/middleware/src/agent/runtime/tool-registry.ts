import type { AgentTool, DeviceInfo, RegisteredTool } from './types.js';

export type ToolRegistry = ReturnType<typeof createToolRegistry>;

export const createToolRegistry = () => {
  const tools: Map<string, Map<string, AgentTool>> = new Map();
  ...
  const getToolsForDevice = (deviceId: string): AgentTool[] => {
    const deviceTools = tools.get(deviceId);
    if (!deviceTools) {
      return [];
    }
  ...
  const getAggregatedTools = (): AgentTool[] => {
