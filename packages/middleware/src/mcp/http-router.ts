...
import type { MCPMessageHandler } from './handler.js';
...
export const mapToolCallError = (
  error: Error,
): { status: number; code: MCPErrorCode } => {
...
export const createMCPRouter = (handler: MCPMessageHandler): Router => {
  ...
  router.post('/tool-call', async (req, res) => {
    ...
    try {
      ...
      const err = error instanceof Error ? error : new Error('Internal MCP error');
