...
- Known tools:
  ...
  `disable` disables buffering.
  `clearMessages` clears buffered logs.
  `getMessages` returns paginated logs with filters.
...
- For log reads, prefer `getMessages` with bounded `limit` and continue with returned `cursor`.
