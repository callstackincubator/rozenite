import { createHttpAgentClient } from './http-client.js';
...
const assertSupportedReactNativeVersion = (target: MetroTarget): void => {
  ...
  if (!target.reactNativeVersion) {
    throw new Error(
      `Rozenite Agent requires React Native >= ${MIN_SUPPORTED_RN_LABEL}. Unable to detect React Native version on device "${target.name}". Upgrade React Native to >= ${MIN_SUPPORTED_RN_LABEL} and restart Metro/DevTools.`,
    );
  }

  throw new Error(
    `Rozenite Agent requires React Native >= ${MIN_SUPPORTED_RN_LABEL}. Detected ${target.reactNativeVersion} on device "${target.name}". Upgrade React Native and restart Metro/DevTools.`,
  );
};
...
export const getMetroTargets = async (
  ...
  const client = createHttpAgentClient(host, port);
  try {
...
export const resolveTargetDeviceId = async (
  ...
  if (targets.length > 1) {
    throw new Error(
      'Multiple connected devices detected. Run `rozenite agent targets --json` and provide --deviceId <id>.',
    );
  }
