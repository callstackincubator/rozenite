...
import { registerDaemon, touchRegisteredDaemon, unregisterDaemon } from './daemon-registry.js';
...
export const createAgentDaemonServer = (options: DaemonOptions) => {
  ...
  const shutdown = async (): Promise<number> => {
    ...
    sessions.clear();
    unregisterDaemon(options.workspace);
    removeDaemonStateArtifacts();
    return stoppedSessions;
  };

  const scheduleShutdown = (reason: string): void => {
    if (shutdownRequested || shutdownInFlight) {
      return;
    }
    shutdownRequested = true;
    logger.info('Scheduling daemon shutdown', {
      reason,
      sessionCount: sessions.size,
    });
  };

  const maybeShutdownWhenIdle = (reason: string): void => {
    if (sessions.size === 0) {
  ...
  const handle = async (request: RPCRequestEnvelope): Promise<unknown> => {
    ...
    switch (request.method) {
      ...
      case 'session.create': {
        ...
        const session = createDaemonSession(
          ...
          (sessionId) => {
            ...
            maybeShutdownWhenIdle('session-terminated');
          },
          logger,
        );
        await session.start();
      ...
      case 'session.stop': {
        ...
        sessions.delete(sessionId);
        writeMetadataSafely();
        maybeShutdownWhenIdle('session-stopped');
  ...
  const handleConnectionLine = async (line: string, socket: net.Socket): Promise<void> => {
    ...
    if (!shutdownRequested) {
      maybeShutdownWhenIdle(`request:${request.method}`);
    }
    if (shutdownRequested) {
