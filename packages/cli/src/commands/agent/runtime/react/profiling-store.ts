  ...
  return {
    ...
    getStatus: (rootsCount: number): {
      ...
      return {
        ...
        isProcessingData: state.session.phase === 'stopping' || state.session.phase === 'processing',
    ...
    getCommitData: (rootId: number, commitIndex: number): ReactCommitData => {
      if (state.session.conflictingRootIds.has(rootId)) {
        throw new Error(`Commit data for root "${rootId}" is ambiguous across multiple React renderers.`);
      }
