...
const getFirstPartyPluginIds = (): string[] => {
  const entries: { npmUrl: string }[] = JSON.parse(readFileSync(PLUGIN_DIRECTORY_PATH, 'utf-8'));
...
describe('agent domain utils', () => {
  it('derives short domain names for every package in the plugin directory', () => {
    const expected: Record<string, string> = {
      ...
      '@react-native-nitro-geolocation/rozenite-plugin': 'react-native-nitro-geolocation',
  ...
  it('is a pure function of the plugin id: a rival package cannot rename an official plugin', () => {
    ...
    expect(deriveDomainName('rozenite-mmkv-plugin')).toBe('rozenite-mmkv-plugin');
  });
  ...
  it('never lets a plugin shadow a built-in domain name', () => {
    ...
    expect(deriveDomainName('rozenite-react-plugin')).toBe('rozenite-react-plugin');
    expect(deriveDomainName('@rozenite/react-plugin')).toBe('@rozenite/react-plugin');
  });
  ...
  it('infers plugin ids for scoped, unscoped, and empty-style names', () => {
    expect(inferPluginId('@rozenite/mmkv-plugin.list-entries')).toBe('@rozenite/mmkv-plugin');
  ...
  it('infers short tool names from tool names', () => {
    expect(inferToolShortName('@rozenite/mmkv-plugin.list-entries')).toBe('list-entries');
  ...
  it('errors instead of silently renaming when two packages by the same author derive the same domain name', () => {
    expect(() =>
      buildRuntimePluginDomains([tool('@a/rozenite-plugin-x.list'), tool('@a/x-plugin.list')]),
    ).toThrow(/Ambiguous domain name "a\/x": plugins @a\/rozenite-plugin-x, @a\/x-plugin/);
  });
  ...
  it('resolves plugin domains by id or pluginId token', () => {
    const domains = buildRuntimePluginDomains([tool('@rozenite/mmkv-plugin.list-entries')]);
  ...
  it('tolerantly resolves mmkv, rozenite/mmkv, and the full package name to the same domain', () => {
    ...
    expect(resolveDomainToken('mmkv', domains)?.pluginId).toBe('@rozenite/mmkv-plugin');
    expect(resolveDomainToken('rozenite/mmkv', domains)?.pluginId).toBe('@rozenite/mmkv-plugin');
  ...
  it('still accepts the deprecated pre-#319 mangled slug as a compatibility alias', () => {
    ...
    expect(resolveDomainToken('at-rozenite__mmkv-plugin', domains)?.pluginId).toBe(
      '@rozenite/mmkv-plugin',
    );
  });
  ...
  it('uses origin-aware descriptions for plugin and app domains', () => {
    ...
    const pluginDomain = domains.find((domain) => domain.pluginId === '@rozenite/mmkv-plugin');
  ...
  it('resolves domain tools by full name or short name', () => {
    ...
    expect(
      resolveDomainTool(tools, '@rozenite/mmkv-plugin', '@rozenite/mmkv-plugin.list-entries').name,
    ).toBe('@rozenite/mmkv-plugin.list-entries');

    expect(resolveDomainTool(tools, '@rozenite/mmkv-plugin', 'remove-entry').name).toBe(
      '@rozenite/mmkv-plugin.remove-entry',
    );
  });

  it('throws precise errors for ambiguous or missing tools', () => {
    expect(() =>
      resolveDomainTool([tool('app.alpha.echo'), tool('app.beta.echo')], 'app', 'echo'),
    ).toThrow('Ambiguous tool "echo" for domain "app". Matches: app.alpha.echo, app.beta.echo.');

    expect(() => resolveDomainTool([tool('app.echo')], 'app', 'missing')).toThrow(
