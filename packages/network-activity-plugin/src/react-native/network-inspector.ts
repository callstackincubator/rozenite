...
const getResponseBody = async (request: XMLHttpRequest): Promise<{ body: string, base64Encoded: boolean }> => {
  try {
...
const getInitiatorFromStack = (): { type: string; url?: string; lineNumber?: number; columnNumber?: number } => {
  try {
...
export const getNetworkInspector = (pluginClient: NetworkActivityDevToolsClient): NetworkInspector => {
  ...
  const enable = () => {
    ...
    XHRInterceptor.setOpenCallback((method: string, url: string, request: XMLHttpRequest) => {
      ...
      networkRequestsRegistry.addEntry(requestId, request, {
        id: requestId,
        loaderId,
        documentURL: typeof document !== 'undefined' ? document.URL : undefined,
        method,
        url,
    ...
    XHRInterceptor.setHeaderReceivedCallback((responseContentType: string | void, responseSize: number | void, allHeaders: string, request: XMLHttpRequest) => {
      const requestId = findRequestId(request);
      if (!requestId) return;
    ...
    XHRInterceptor.setResponseCallback((status: number, timeout: number, response: string, responseURL: string, responseType: string, request: XMLHttpRequest) => {
      const requestId = findRequestId(request);
      if (!requestId) return;
  ...
  const handleBodySubscription = pluginClient.onMessage('Network.getResponseBody', async (payload) => {
