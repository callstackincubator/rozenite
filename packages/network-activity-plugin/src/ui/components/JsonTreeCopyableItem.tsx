...
export const JsonTreeCopyableItem = ({
  ...
  return (
    <span className={cn('inline-block group', className)}>
      {children}
      <div
        className="inline-block cursor-pointer opacity-0 group-hover:opacity-100 text-muted hover:text-foreground/70 transition-all p-2 -m-2 ml-0 translate-y-0.75"
        onClick={handleCopy}
      >
