...
const wireChromeListeners = (ext: ReturnType<typeof createExtensionWithStubs>) => {
  (
    ...
    ext.pageManager.onNavigationCompleted(
      args[0] as Parameters<typeof ext.pageManager.onNavigationCompleted>[0],
    ),
  );
  (
    globalThis as { chrome?: ReturnType<typeof createChromeStub> }
...
describe('Extension integration', () => {
  ...
  beforeEach(() => {
    ...
    chromeStub.configure.setScriptingResult({
      appDisplayName: 'Test',
  ...
  it('socket created on first page', async () => {
    ...
    assert.strictEqual(
      ...
      1,
      'Exactly one WebSocket should be created',
    );
    assert.ok(
  ...
  it('different origin gets its own socket', async () => {
    ...
    assert.ok(
      urls.some((u) => u.includes('localhost:8081')),
      'One socket for 8081',
    );
    assert.ok(
