...
const createMockPageManager = (
  pages: { id: string; reactNativeMetadata?: Record<string, unknown> }[] = [],
) => {
  return {
    getAll: () => pages,
  };
...
describe('ReactNativeAgent', () => {
  ...
  it('handleCommand enables agent and sends metadata', () => {
    ...
    assert.strictEqual(
      (responses[1] as { method: string }).method,
  ...
  it('handleCommand returns error for unknown ReactNativeApplication method', () => {
    ...
    agent.handleCommand(
      'page1',
      { id: 1, method: 'ReactNativeApplication.unknownMethod' },
      sendResponse,
    );
  ...
  it('getMetadata uses default values when page not found', () => {
    ...
    const metadata = (
      responses[1] as {
        params: { appDisplayName: string; reactNativeVersion: string };
      }
    ).params;
