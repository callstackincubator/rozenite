/**
 * The public entry point: an rspeedy/Rsbuild plugin that wires
 * `./transport/` (DebugRouter discovery) and `./server/` (the
 * transport-agnostic `/json/list` + `/inspector/debug` HTTP/WS half) into
 * a Lynx dev server — the same shape `@rozenite/metro`'s `withRozenite`
 * gives Metro, and `@rozenite/repack`'s `withRozenite` gives Re.Pack.
 *
