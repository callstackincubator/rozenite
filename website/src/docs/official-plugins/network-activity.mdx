...
- **Images** (`image/png`, `image/jpeg`, `image/gif`, `image/webp`, `image/svg+xml`, ...) render inline in the Preview tab via a data URL. The Raw tab shows a metadata card with Content-Type and decoded size; for SVG specifically, the Raw tab shows the source XML.
- **JSON** renders as a collapsible tree with copy affordances on every key and value.
- **Text and other source formats** (`text/*`, `application/xml`, `application/javascript`, ...) render as raw monospace text.
- **Empty or oversized responses** show a clear placeholder. Image responses above an in-capture 5 MB cap surface as "Response too large for preview" instead of crossing the bridge.
