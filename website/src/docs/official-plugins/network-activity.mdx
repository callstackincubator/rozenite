...
:::info Not agent-compatible
This plugin provides a network inspector for the React Native DevTools UI. It does not currently expose agent-compatible tools and cannot be used with `rozenite agent`. If you need network inspection through the agent interface, use the built-in `network` domain — which requires **React Native 0.83 or later**.
:::
...
To capture network requests happenning before your React Native app initialization, add this to your entrypoint:
