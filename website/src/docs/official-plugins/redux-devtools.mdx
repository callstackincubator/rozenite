...
When running under Hermes, if you App component initialization is delayed for example with `require()`, we could potentially hit an undefined `asyncIterator`, not see the log `connected to remotedev-server`, and not see the physical device in the inspector's selector.

Importing Rozenite in your `index.js`
```
import '@rozenite/redux-devtools-plugin/dist/runtime.cjs'
```

Else try the workaround of this [issue](https://github.com/reduxjs/redux-devtools/issues/1382#issuecomment-2093584916).
