...
When running under Hermes, some could potentially hit an undefined `asyncIterator`, not see the log `connected to remotedev-server`, and not see the physical device in the inspector's selector.
