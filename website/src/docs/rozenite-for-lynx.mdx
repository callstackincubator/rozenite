...
Rozenite for Lynx is experimental. It has been verified against LynxExplorer on the iOS Simulator, the Android emulator and a physical Android phone, not across the full range of LynxSDK versions and platforms, and the API may change in future releases.
:::
...
**Your targets all disappear at once, and stay gone.** Something else attached to DebugRouter and took the connections — a second Rozenite dev server is the usual culprit. Only one tool can talk to a device's DebugRouter at a time. Reloading the app won't bring the targets back; close the other tool and restart the dev server.
