...
Rozenite ships a single skill, `rozenite`. It is a thin router: it tells the agent to discover and read the real content through the `rozenite skills` CLI command instead of hardcoding a doc inventory that could go stale.
